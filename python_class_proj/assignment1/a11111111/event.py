...
class Arrive(Event):
    ...
    def do(self, store):
        ...
        if store.count_line[line_num].costumer_waiting_list.is_empty():
            ...
            spawned_event = BeginCheck(self.timestamp, line_num, self.costumer)
...
class BeginCheck(Event):
    ...
    """
    def __init__(self, timestamp, count_num, costumer):
    ...
    def do(self, store):
        ...
        return [[finish_timestamp, FinishCheck(finish_timestamp, self.count_num, self.costumer)]]
        # return single event.
...
class FinishCheck(Event):
    ...
    def do(self, store):
        ...
        @type store: GroceryStore.
        @rtype: next BeginCheck or None.
        """
        line_num = self.count_num
        store.count_line[line_num].costumer_waiting_list.dequeue()
        # pop a costumer.
        if not store.count_line[line_num].costumer_waiting_list.is_empty():
            next_costumer = store.count_line[line_num].costumer_waiting_list.returnfirst()
            return [[self.timestamp, BeginCheck(self.timestamp, line_num, next_costumer)]]
        else:
            return None
        # return single event.
