...
class New_customer(Event):
    def __init__(self,timestamp,name,num_of_items):
        Event.__init__(self,timestamp)
        self.name=name
        self.num_of_items=num_of_items
        self.type='New'
        self.type_of_line=None
        self.num_of_line=-1
 
class Begin_checking(Event):
    def __init__(self,timestamp,name,num_of_items,type_of_line,num_of_line):
        Event.__init__(self,timestamp)
        self.name=name
        self.num_of_items=num_of_items
        self.type='Check'
...
def create_event_list(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            # Create a list of words in the line, e.g. ['60', 'Arrive', 'Bob', '5'].
            # Note that these are strings, and you'll need to convert some of them
            # to ints.
            tokens = line.split()
            if tokens[1]=='Arrive':
                events.append(New_customer(int(tokens[0]),tokens[2],int(tokens[3])))
            elif tokens[1]=='Check':
                events.append(Begin_checking(int(tokens[0]),tokens[2],int(tokens[3])))
            elif tokens[1]=='Finish':
                events.append(Finish_checking(int(tokens[0]),tokens[2],int(tokens[3])))
            else:
                events.append(Line_closed(int(tokens[0]),int(tokens[2])))
