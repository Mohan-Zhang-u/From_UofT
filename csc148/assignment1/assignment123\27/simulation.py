...
class GroceryStoreSimulation:
    ...
    def run(self, event_file):
        ...
        # initial_events is a PriorityQueue of [item.timestamp, item](存储了分割转化好的Event)
        while not initial_events.is_empty():
            todo = initial_events.remove()
            #todo是将要做的([item.timestamp, item])
            print('todo,', todo)
            #test
            ...
            initial_events.printall()
            print('once')
            #test
            #spawn 是todo.do了之后返回的[[item.timestamp, item],[item.timestamp, item]]...
            if isinstance(spawn,list):
                for i in spawn:
                    initial_events.add(i)
                #spawn 定制完成,添加了[item.timestamp, item]在PriorityQueue里.
