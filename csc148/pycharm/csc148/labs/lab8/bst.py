class BinarySearchTree:
    ...
    def width(self):
        """Return the width of this BST.

        width is the max number of items at any level in the tree.

        @type self: BinarySearchTree
        @rtype: int

        >>> BinarySearchTree(None).width()
        0
        >>> bst = BinarySearchTree(7)
        >>> bst.width()
        1
        ...
        >>> bst.width()
        2
        >>> bst._left._right = BinarySearchTree(6)
        >>> bst.width()
        2
        >>> bst._right._left = BinarySearchTree(8)
        >>> bst.width()
        3
        >>> bst._right._right = BinarySearchTree(10)
        >>> bst.width()
        4
        """
        # TODO
        lst = self.width_helper()
        return max(lst)


    def width_helper(self):
        """Return the the list used of this BST.
        ...
        if self.is_empty():
            ...
            return [1] + self._right.width_helper()
        elif not self._left.is_empty() and self._right.is_empty():
            return [1] + self._left.width_helper()
        elif not self._left.is_empty() and not self._right.is_empty():
            return [1] + add_two_list(self._left.width_helper(), self._right.width_helper())

def add_two_list(x, y):
    re_list = []
    for i in range(min(len(x), len(y))):
        re_list.append(x[i]+y[i])
    if max(len(x), len(y)) == len(x):
        re_list = re_list + x[len(y):]
    else:
        re_list = re_list + y[len(x):]
    return re_list
