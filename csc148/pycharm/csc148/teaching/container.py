...
class PriorityQueue(Container):
    ...
    def add(self, item):
        ...
        ['monalisa', 'arju', 'fred']
        """
        # check the priority whether it is empty. Otherwiese, no priority
        # happens.
        if self.is_empty():
            self._queue.append(item)
        # if it is not empty, compare the priority of new item with each item
        # starting with index 0 with the item needed to be added
        else:
            i = 0
            for ele in self._queue:
                if self._less_than(item, ele):
                    i += 1
            self._queue.insert(i, item)
            # Until finding a item with bigger priority, insert new item at the
            # the index of the item.
            # The item with bigger priority will go to next index.
