...
// find the branch that answer reversed, starting from
// node current
QNode *find_branch2(QNode *current, int answer){
    answer=1-answer;
    return current->children[answer].qchild;   
}
...
// find a user in the tree according to the reversed answers
// return NULL if the user was not found
// return a linked list of friends
Node *find_user_according_to_reversed_answer(QNode *current, char *name){
    QNode *curr2=current;   
    while (current->node_type == REGULAR){
        if (find_user(current->children[0].qchild, name)!=NULL){
            curr2=curr2->children[1].qchild;
            current=current->children[0].qchild;
        }
        else if (find_user(current->children[1].qchild, name)!=NULL){
            curr2=curr2->children[0].qchild;
    ...
    // look for the user in the 0 fchild       
    Node *head = current->children[0].fchild;
    while (head != NULL) {
        if (strcmp(head->str, name) == 0)
            return curr2->children[1].fchild;
        head = head->next;
    }
    ...
    while (head != NULL) {
        if (strcmp(head->str, name) == 0)
            return curr2->children[0].fchild;
        head = head->next;
    }
    return NULL;
}
