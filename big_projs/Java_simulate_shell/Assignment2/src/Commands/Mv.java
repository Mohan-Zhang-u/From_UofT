...
public class Mv extends Command {
   ...
   */
  public void execute() {
    if (!validMvCommandKeywords()){
      return;
    }
  }

  // check whether the mv command has 2 parameters
  public boolean validMvCommandKeywords() {
    if (commandKeywords.length !=2){
      System.out.println("Error: Mv command must take exactly 2 commands");
      return false;
    }
    else {
      ...
      if (fs.cangetDirByPath(commandKeywords[1])&&
        ...
        if (newDir.getStringPath().contains(oldDir.getStringPath())){
          System.out.println("Error:Invalid argument, cannot mv a directory"
              + " to its subdirectory");
          return;
        }
      }
